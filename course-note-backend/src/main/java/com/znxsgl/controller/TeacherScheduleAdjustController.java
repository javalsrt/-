 ...
 *      原 "/api/teacher/schedule" 改为 "/teacher/schedule"，对外实际 URL 仍是 /api/teacher/schedule/...
 *   2) 本任务不迁移"调课 WebSocket 推送通知"（ScheduleNotifyService 未迁入），
 *      因此移除了调课完成后的即时通知逻辑，仅保留"调课 + 锁定 + 查询"核心能力。
 */
@RestController
...
public class TeacherScheduleAdjustController {
     ...
     * 配合前端生成的唯一 requestId，可防止网络重试或用户快速点击导致的重复调课。
     */
    ...
    public ResponseEntity<Map<String, Object>> adjustSchedule(
        ...
        // 对同一 requestId 加锁，保证并发场景下 check + put 原子性，防止重复调课
        ...
        // 适配：本任务不迁移"调课 WebSocket 推送通知（ScheduleNotifyService）"，此处省略即时通知逻辑
        // （调课 + 锁定已在上方完成，仅不推送通知而已）。

        // 调课成功后记录幂等键，防止重复提交导致重复调课
