 ...
 *    原 "/api/schedule" 改为 "/schedule"，对外实际 URL 仍是 /api/schedule/...
 *    ⚠️ Bean 名：com.coursenote.controller 也存在同名 ScheduleController（小林既有占位），
 *    组件扫描默认 bean 名均为 scheduleController 会冲突，故此处显式指定 bean 名。
 */
@RestController("znxsglScheduleController")
...
public class ScheduleController {
     ...
     *
     * 参照 aiStudy 安卓端显示逻辑：假期（学期未开学/已结束）时不隐藏课表——
     * 当前学期无课表数据时，自动回退到最近一个有课表数据的学期，
     * 保证"管理员导入了课程，学生假期也能查看课表"。
     *
     * 前端根据 semester.status 显示：
     * - "before" / "ended" → 顶部显示 notice 提示条，课表正常展示
    ...
    public ResponseEntity<Map<String, Object>> getMyScheduleWithSemester(Authentication auth,
        ...
        // 未指定学期且当前学期无课表数据 → 回退到最近有课表数据的学期（参照 aiStudy 安卓端）
        boolean autoPick = semester == null || semester.trim().isEmpty();
        if (autoPick && schedules.isEmpty()) {
            String fallback = scheduleService.getLatestSemesterWithSchedule(userId);
            if (fallback != null) {
                semesterDto = semesterService.getSemesterWithStatusByName(fallback);
                schedules = scheduleService.getStudentSchedule(userId, week, fallback);
            }
        }
