...
/**
 * AI 出题 / 能力评估接口（自 aiStudy 迁移）。
 * 适配说明：
 *  1. context-path=/api：控制器映射基于应用内路径（不含 /api 前缀），对外 URL 为 /api/quiz/...。
 *  2. 已去除“章节依赖”：原逻辑要求先学章节并通过 chapter_read_progress 校验、走第一章 RAG 检索；
 *     现改为以 znxsgl 的 course.description + 小林 knowledge_document（按 user_id 归属）作为出题素材。
 */
@RestController
@RequestMapping("/quiz")
public class QuizController {
    ...
    /** 生成题目（自适应：按课程难度档位 + 课程描述/知识文档出题，无章节强校验） */
    ...
    public ResponseEntity<Map<String, Object>> generate(@RequestBody Map<String, Object> body, Authentication auth) {
        ...
        // 出题素材：课程描述 + 该学生的知识文档（去章节依赖，不校验学习进度）
        String materialContext = buildMaterialContext(courseId, userId);

        String prompt = buildAdaptivePrompt(subject, subjectType, difficulty, materialContext);
        ...
        session.setChapterScope(null); // 去章节化，不再记录章节范围
    ...
    /**
     * 组装出题素材：目标库 znxsgl.course.description + 小林库 knowledge_document.content（按 user_id 归属）。
     * 不再依赖 chapter_read_progress / course_chapter，不强制“先学章节”。
     * 素材为空时不阻塞，交由 LLM 按科目泛化出题。
     */
    private String buildMaterialContext(Long courseId, Long userId) {
        StringBuilder sb = new StringBuilder();
        // 1) 课程描述
        try {
            String desc = jdbc.queryForObject(
                    "SELECT description FROM course WHERE id = ?", String.class, courseId);
            if (desc != null && !desc.trim().isEmpty()) {
                sb.append("[课程描述]\n").append(desc.trim()).append("\n\n");
            }
        } catch (Exception e) {
            System.out.println("=== 出题素材: 读取课程描述失败: " + e.getMessage());
        }
        // 2) 该学生的知识文档（排除逻辑删除）
        try {
            List<String> contents = jdbc.queryForList(
                    "SELECT content FROM knowledge_document " +
                    "WHERE user_id = ? AND deleted = 0 AND content IS NOT NULL AND content <> '' " +
                    "ORDER BY created_at DESC LIMIT 20", String.class, userId);
            for (int i = 0; i < contents.size(); i++) {
                String c = contents.get(i);
                if (c != null && !c.trim().isEmpty()) {
                    sb.append("[文档素材 ").append(i + 1).append("]\n").append(c.trim()).append("\n\n");
                }
            }
        } catch (Exception e) {
            System.out.println("=== 出题素材: 读取知识文档失败: " + e.getMessage());
        }
        return sb.toString();
    }
    ...
    private String buildAdaptivePrompt(String subject, String type, int difficulty, String materialContext) {
        String difficultyDesc;
        String difficultyReq;
        if (difficulty == 1) {
            difficultyDesc = "基础档（概念定义和基本事实记忆题，答案能直接在素材内容中找到）";
            difficultyReq = "题目以概念定义和基本事实记忆为主，答案能直接在素材内容中找到";
        ...
        return String.format(
                ...
                "科目：《%s》（%s类）\n" +
                "本次出题难度档位：%s\n" +
                "出题范围：优先基于以下课程资料与知识文档内容出题，超出范围的部分可结合该科目常识合理出题：\n" +
                "---\n%s\n---\n" +
                "要求：\n" +
                "- %s\n" +
                "- 选择题4选项，正确答案末尾加*\n" +
                ...
                subject, type, difficultyDesc, materialContext, difficultyReq,
    ...
    /**
     * 复习加强：返回最近一次测评的薄弱点、学习计划（已去除 recommendedChapters 章节推荐）
     */
    ...
    public ResponseEntity<Map<String, Object>> reviewPlan(Authentication auth) {
        ...
        try {
            ...
            Map<String, Object> result = new LinkedHashMap<>();
            result.put("subject", latest.get("subject") != null ? latest.get("subject").toString() : "");
            ...
            // 用 HashMap（允许 null 值），避免 Map.of 对 null courseId 抛 NPE
            Map<String, Object> fallback = new HashMap<>();
            fallback.put("subject", "");
            fallback.put("courseId", null);
            fallback.put("accuracy", "0%");
            fallback.put("scores", new HashMap<>());
            fallback.put("weaknesses", new ArrayList<>());
            fallback.put("suggestion", "暂无测评记录，先去专注刷题完成一次测评吧。");
            fallback.put("studyPlan", new ArrayList<>());
