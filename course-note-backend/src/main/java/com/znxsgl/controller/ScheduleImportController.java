 ...
 *
 * ⚠️ 适配：
 *   1) 本项目 context-path=/api，controller 映射基于应用内路径（不含 /api 前缀）。
 *      原 "/api/schedule/import" 改为 "/schedule/import"，对外实际 URL 仍是 /api/schedule/import/...
 *   2) 本任务不迁移调课 WebSocket 推送通知（ScheduleNotifyService），故移除导入成功后的通知逻辑。
 *   3) aiStudy 依赖 LlmService（LLM 提取课表）做非标准格式解析，本任务未迁移该能力：
 *      仅支持标准 Excel 模板（schedule-import-template.xlsx）解析导入；非标准格式会返回友好提示。
 */
@RestController
@RequestMapping("/schedule/import")
...
public class ScheduleImportController {
    ...
    public ScheduleImportController(JdbcTemplate jdbc,
    ...
    public ResponseEntity<Map<String, Object>> preview(
        ...
        try {
            ...
            if (items == null) {
                // 适配：aiStudy 的 LLM 课表提取能力（LlmService）未随本任务迁移，
                // 仅支持标准 Excel 模板解析；非标准格式直接给出友好错误，不调用外部 AI。
                Files.deleteIfExists(saved);
                return ResponseEntity.ok(Map.of("error",
                    "未能解析课表，请使用系统提供的标准 Excel 模板（schedule-import-template.xlsx）导入"));
            } else {
                Files.deleteIfExists(saved);
            }
    ...
    private ResponseEntity<Map<String, String>> doUnhide(Map<String, Object> body, Authentication auth) {
        ...
        if (selectedSlots != null && !selectedSlots.isEmpty()) {
            ...
            // 适配：本任务不迁移"课表导入成功后的即时通知"（ScheduleNotifyService 未迁入），此处不发送通知
