...
import com.znxsgl.mapper.SysUserRoleMapper;
 ...
 *    原 "/api/admin/user" 改为 "/admin/user"，对外实际 URL 仍是 /api/admin/user/...。
 */
@RestController
...
public class AdminUserController {
    ...
    public ResponseEntity<Map<String, Object>> importStudents(@RequestParam("file") MultipartFile file) {
        ...
        for (User u : allUsers) {
            // 适配：用户表合并后可能存在开放注册账号（openid 登录）无 username 的记录，跳过避免 NPE
     ...
     * 获取所有班级列表
     *
     * 排课管理对教师开放（教师承担本班排课职责），班级下拉/名称映射需要此接口，
     * 因此单独放开 TEACHER 访问（类级注解仍为 ADMIN）。
     */
    @GetMapping("/classes")
    ...
    /**
     * 查询教师所教课程及班级列表（排课管理「一键生成教学任务」数据源）
     *
     * @param teacherUserId 教师对应用户表ID（user.id），可选：
     *                      未传时教师角色查本人课程、管理员查全部课程（供批量生成教学任务）
     * @param semester      学期名称，可选过滤
     *
     * 响应为按课程聚合的结构（camelCase），classes 内含该课程关联的全部班级：
     * [{ courseId, courseName, teacherId, credit, semester, classes: [{ classId, className }] }]
     */
    ...
    public ResponseEntity<List<Map<String, Object>>> getTeacherCourses(
        ...
        // 目标教师ID：null 表示不限教师（管理员批量场景）
        Long targetTeacherId = null;

        if (teacherUserId != null) {
            // 显式指定教师：user.real_name → teacher.id
            ...
            targetTeacherId = teacher.getId();
        } else {
            // 未指定：教师 → 本人课程；管理员 → 全部课程
            ...
            if (currentUser != null && currentUser.getRole() != null && currentUser.getRole() == 2) {
                Teacher self = teacherMapper.selectOne(
                        new LambdaQueryWrapper<Teacher>().eq(Teacher::getRealName, currentUser.getRealName()));
                if (self == null) {
                    return ResponseEntity.ok(Collections.emptyList());
                }
                targetTeacherId = self.getId();
            }
        }

        // 查询课程及关联班级（扁平行，后续按课程聚合）
        String sql = "SELECT c.id AS course_id, c.course_name, c.credit, c.semester, c.teacher_id, " +
        ...
        // 按 courseId 聚合班级，输出 camelCase（与前端 generateTasksFromCourses 消费结构对齐）
        Map<Long, Map<String, Object>> byCourse = new LinkedHashMap<>();
        for (Map<String, Object> row : rows) {
            Long courseId = ((Number) row.get("course_id")).longValue();
            Map<String, Object> course = byCourse.get(courseId);
            if (course == null) {
                course = new LinkedHashMap<>();
                course.put("courseId", courseId);
                course.put("courseName", row.get("course_name"));
                course.put("teacherId", row.get("teacher_id") != null
                        ? ((Number) row.get("teacher_id")).longValue() : null);
                course.put("credit", row.get("credit"));
                course.put("semester", row.get("semester"));
                course.put("classes", new ArrayList<Map<String, Object>>());
                byCourse.put(courseId, course);
            }
            Map<String, Object> cls = new LinkedHashMap<>();
            cls.put("classId", ((Number) row.get("class_id")).longValue());
            cls.put("className", row.get("class_name"));
            ((List<Map<String, Object>>) course.get("classes")).add(cls);
        }

        return ResponseEntity.ok(new ArrayList<>(byCourse.values()));
    }
}