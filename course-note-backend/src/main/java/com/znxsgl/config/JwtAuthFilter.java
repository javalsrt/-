...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            // ⚠️ 兼容适配：合并到小林后，小林既有用户通过 /api/user/login、/api/teacher/login 走 openid 登录，
            //    其 current_token 字段为 NULL（不经过本鉴权模块）。此时仅校验用户存在，不做单设备强校验，
            //    小林的并发鉴权由 com.coursenote JwtInterceptor 负责；znxsgl 用户置于 /api/auth/login 后会写库，
            //    依旧享受单设备登录保护。
            if (user == null || (user.getCurrentToken() != null && !token.equals(user.getCurrentToken()))) {
                // token 已被新登录覆盖（或账号不存在），返回 401
                response.setStatus(401);
                response.setContentType("application/json;charset=UTF-8");
