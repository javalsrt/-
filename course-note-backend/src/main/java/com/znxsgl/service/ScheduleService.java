...
public class ScheduleService {
    ...
    /**
     * 查询该学生最近一个有课表数据的学期名（按学期开学日期倒序，学期表无记录时按学期名倒序）。
     * 参照 aiStudy 安卓端：假期/未开学时当前学期无课表数据，回退显示最近有数据的学期，
     * 保证"管理员导入了课程，学生假期也能查看课表"。
     */
    public String getLatestSemesterWithSchedule(Long userId) {
        try {
            List<Map<String, Object>> rows = jdbc.queryForList(
                "SELECT s.semester FROM schedule s " +
                "LEFT JOIN semester sem ON sem.name = s.semester " +
                "WHERE s.user_id = ? AND s.status = 1 AND s.day_of_week > 0 " +
                "GROUP BY s.semester " +
                "ORDER BY COALESCE(sem.start_date, '1970-01-01') DESC, s.semester DESC " +
                "LIMIT 1", userId);
            return rows.isEmpty() ? null : (String) rows.get(0).get("semester");
        } catch (Exception e) {
            log.warn("查询学生最近有课表的学期失败 userId={}", userId, e);
            return null;
        }
    }
    ...
    public List<StudentCourseDTO> getStudentCourses(Long userId) {
        ...
        for (Map<String, Object> row : rows) {
            ...
            // 注：chat_message 属于 aiStudy 课堂助手扩展表，本迁移未建该表时容错返回 0
            int unread = 0;
            try {
                unread = jdbc.queryForObject(
                    "SELECT COUNT(*) FROM chat_message " +
                    "WHERE course_name = ? AND user_id = ? AND sender_role = 'teacher' AND is_read = 0",
                    Integer.class, dto.getCourseName(), userId);
            } catch (Exception ex) {
                log.warn("查询课程未读消息数失败（chat_message 表可能未迁移）: courseName={}", dto.getCourseName(), ex);
            }
            ...
            // 注：exam_homework/exam_submission 属于 aiStudy 课堂助手扩展表，本迁移未建表时容错返回空列表
            List<Map<String, Object>> pendingExams = new ArrayList<>();
            try {
                pendingExams = jdbc.queryForList(
                ...
                log.warn("查询课程考试/作业失败（exam_homework 等表可能未迁移）: courseId={}", dto.getCourseId(), ex);
            }
