...
public class AutoScheduleService {
     ...
     * @param teacherId 指定教师ID（仅排该教师的任务）；null 表示排全部（管理员）
     * @return 排课结果统计
     */
    @Transactional
    public ScheduleResult autoSchedule(String semester, boolean clearExisting, Long teacherId) {
        ScheduleResult result = new ScheduleResult();
        long t0 = System.currentTimeMillis();

        // 1. 加载教学任务：全量重排时纳入已排任务（scheduled），否则只取待排 + 失败的
        LambdaQueryWrapper<TeachingTask> taskQuery = new LambdaQueryWrapper<TeachingTask>()
                .eq(TeachingTask::getSemester, semester);
        if (!clearExisting) {
            taskQuery.in(TeachingTask::getStatus, "pending", "failed");
        }
        if (teacherId != null) {
            taskQuery.eq(TeachingTask::getTeacherId, teacherId);
        }
        List<TeachingTask> tasks = teachingTaskMapper.selectList(taskQuery.orderByAsc(TeachingTask::getPriority));
        ...
        // 2. 收集导入课程自带的教室（course_id → classroom）。
        //    必须在清空 schedule 之前执行：导入课程表时上传的教室位置是排课的首选依据，
        //    这样管理员无需预先在"教室管理"中新建教室。
        Map<Long, String> importedClassrooms = collectImportedClassrooms(semester, teacherId);

        // 2.5 清空已有的非锁定课程，并把纳入重排的任务状态重置为 pending（如果是全量重排）
        if (clearExisting) {
            clearUnlockedSchedules(semester, teacherId);
            List<Long> taskIds = tasks.stream().map(TeachingTask::getId).collect(java.util.stream.Collectors.toList());
            for (Long id : taskIds) {
                teachingTaskMapper.update(null, new LambdaUpdateWrapper<TeachingTask>()
                        .eq(TeachingTask::getId, id)
                        .set(TeachingTask::getStatus, "pending"));
            }
        }
        ...
        for (TeachingTask task : tasks) {
            boolean success = scheduleOneTask(task, ctx, roomsByType, studentsByClass, totalWeeks, roomUsage, dayLoad, importedClassrooms, result);
            if (success) {
                result.scheduled++;
                // 更新任务状态
    ...
    private boolean scheduleOneTask(TeachingTask task,
                                     ...
                                     Map<Long, String> importedClassrooms,
        ...
        // 获取可用教室列表：导入教室优先（无需在教室管理中新建），其次按类型/容量从教室表选择
        List<Classroom> availableRooms = new ArrayList<>();
        String importedRoom = importedClassrooms != null ? importedClassrooms.get(task.getCourseId()) : null;
        if (importedRoom != null && !importedRoom.trim().isEmpty()) {
            // 导入课程表时上传的教室位置，直接作为候选之首（容量不限制）
            Classroom virtual = new Classroom();
            virtual.setName(importedRoom.trim());
            virtual.setType("normal");
            virtual.setCapacity(Integer.MAX_VALUE);
            availableRooms.add(virtual);
        }
        List<Classroom> dbRooms = selectAvailableRooms(task, roomsByType, classSize);
        if (dbRooms.isEmpty() && task.getPreferredRoomType() != null
                && !task.getPreferredRoomType().equals("normal")) {
            // 专用教室不够，回退到普通教室
            dbRooms = roomsByType.getOrDefault("normal", Collections.emptyList()).stream()
                    .filter(r -> r.getCapacity() != null && r.getCapacity() >= classSize)
                    .collect(Collectors.toList());
        }
        availableRooms.addAll(dbRooms);
     ...
     */
    private void clearUnlockedSchedules(String semester, Long teacherId) {
        // 找出锁定的 schedule_id
        ...
        StringBuilder sql = new StringBuilder(
                ...
                "WHERE s.semester = ? AND s.status = 1 ");
    ...
    /**
     * 收集课程导入时上传的教室位置（course_id → classroom）。
     * 在全量重排清空 schedule 之前调用，保证导入的教室信息不丢失。
     */
    private Map<Long, String> collectImportedClassrooms(String semester, Long teacherId) {
        Map<Long, String> map = new HashMap<>();
        StringBuilder sql = new StringBuilder(
                "SELECT DISTINCT s.course_id, s.classroom FROM schedule s " +
                "JOIN course c ON c.id = s.course_id " +
                "WHERE s.semester = ? AND s.status = 1 " +
                "AND s.classroom IS NOT NULL AND TRIM(s.classroom) != '' ");
        List<Object> params = new ArrayList<>();
        params.add(semester);
        if (teacherId != null) {
            sql.append(" AND c.teacher_id = ? ");
        ...
        for (Map<String, Object> row : jdbc.queryForList(sql.toString(), params.toArray())) {
            map.put(((Number) row.get("course_id")).longValue(), (String) row.get("classroom"));
        }
        return map;
    }
